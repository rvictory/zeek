...
void Func::SetScope(zeek::ScopePtr newscope)
	{
...
TraversalCode Func::Traverse(TraversalCallback* cb) const
	...
	zeek::Scope* old_scope = cb->current_scope;
...
Val* Func::Call(val_list* args, zeek::Frame* parent) const
	{
	auto zargs = zeek::val_list_to_args(*args);
	return Invoke(&zargs, parent).release();
	};

zeek::ValPtr BroFunc::Invoke(zeek::Args* args, zeek::Frame* parent) const
	{
	...
	auto f = zeek::make_intrusive<zeek::Frame>(frame_size, this, args);
...
void BroFunc::AddClosure(id_list ids, zeek::Frame* f)
	{
	if ( ! f )
		return;
...
bool BroFunc::StrengthenClosureReference(zeek::Frame* f)
	{
	if ( closure != f )
		return false;
...
void BroFunc::SetClosureFrame(zeek::Frame* f)
	{
	if ( closure )
...
bool BroFunc::UpdateClosure(const broker::vector& data)
	{
	auto result = zeek::Frame::Unserialize(data);
...
broker::expected<broker::data> BroFunc::SerializeClosure() const
	{
	return zeek::Frame::Serialize(closure, outer_ids);
	}
...
: Func(BUILTIN_FUNC)
	...
	const auto& id = zeek::lookup_ID(Name(), GLOBAL_MODULE_NAME, false);
	if ( ! id )
...
zeek::ValPtr BuiltinFunc::Invoke(zeek::Args* args, zeek::Frame* parent) const
	{
#ifdef PROFILE_BRO_FUNCTIONS
	DEBUG_MSG("Function: %s\n", Name());
#endif
...
function_ingredients::function_ingredients(zeek::ScopePtr scope, zeek::detail::StmtPtr body)
	{
