 ...
 */
std::string zeek::current_module = GLOBAL_MODULE_NAME;
std::string& current_module = zeek::current_module;
...
expr:
	...
	|	'$' TOK_ID func_params '='
			{
			func_hdr_location = @1;
			auto func_id = zeek::current_scope()->GenerateTemporary("anonymous-function");
			func_id->SetInferReturnType(true);
			begin_func(std::move(func_id), zeek::current_module.c_str(),
	...
	|	TOK_ID
			{
			set_location(@1);
			auto id = zeek::lookup_ID($1, zeek::current_module.c_str());
...
/*	// CHECK THAT THIS IS NOT GLOBAL.
					id = install_ID($1, zeek::current_module.c_str(),
							false, is_export);
*/
...
enum_body_elem:
		...
		*/
		TOK_ID '=' TOK_CONSTANT opt_deprecated
			...
			if ( $3->GetType()->Tag() != zeek::TYPE_COUNT )
				...
				cur_enum_type->AddName(zeek::current_module, $1,
	...
	|	TOK_ID opt_deprecated
			...
			set_location(@1);
			assert(cur_enum_type);
			cur_enum_type->AddName(zeek::current_module, $1, is_export, $2);
			}
	;
...
decl:
		TOK_MODULE TOK_ID ';'
			{
			zeek::current_module = $2;
			zeekygen_mgr->ModuleUsage(::filename, zeek::current_module);
			}
...
func_hdr:
		TOK_FUNCTION def_global_id func_params opt_attr
			{
			zeek::IntrusivePtr id{zeek::AdoptRef{}, $2};
			begin_func(id, zeek::current_module.c_str(),
	...
	|	TOK_EVENT event_id func_params opt_attr
			...
			begin_func({zeek::NewRef{}, $2}, zeek::current_module.c_str(),
	...
	|	TOK_REDEF TOK_EVENT event_id func_params opt_attr
			{
			begin_func({zeek::NewRef{}, $3}, zeek::current_module.c_str(),
...
lambda_body:
		...
		'}'
			...
			auto ingredients = std::make_unique<function_ingredients>(zeek::IntrusivePtr{zeek::NewRef{}, zeek::current_scope()}, zeek::IntrusivePtr{zeek::AdoptRef{}, $3});
			id_list outer_ids = gather_outer_ids(zeek::pop_scope().get(), ingredients->body.get());
...
begin_func:
		func_params
			{
			auto id = zeek::current_scope()->GenerateTemporary("anonymous-function");
			begin_func(id, zeek::current_module.c_str(), zeek::FUNC_FLAVOR_FUNCTION, 0, {zeek::AdoptRef{}, $1});
			$$ = id.release();
			}
	;
...
event:
		TOK_ID '(' opt_expr_list ')'
			{
			set_location(@1, @4);
			const auto& id = zeek::lookup_ID($1, zeek::current_module.c_str());
...
case_type:
	...
	|	TOK_TYPE type TOK_AS TOK_ID
			...
			auto case_var = zeek::lookup_ID(name, zeek::current_module.c_str());

			if ( case_var && case_var->IsGlobal() )
				case_var->Error("already a global identifier");
			else
				case_var = zeek::install_ID(name, zeek::current_module.c_str(), false, false);
...
for_head:
		TOK_FOR '(' TOK_ID TOK_IN expr ')'
			...
			auto loop_var = zeek::lookup_ID($3, zeek::current_module.c_str());
			...
			if ( loop_var )
				...
				{
				loop_var = zeek::install_ID($3, zeek::current_module.c_str(),
						      false, false);
				}
	...
	|
		TOK_FOR '(' TOK_ID ',' TOK_ID TOK_IN expr ')'
			{
			set_location(@1, @8);
			const char* module = zeek::current_module.c_str();

			// Check for previous definitions of key and
			// value variables.
			auto key_var = zeek::lookup_ID($3, module);
			auto val_var = zeek::lookup_ID($5, module);
			...
			if ( key_var )
				...
				key_var = zeek::install_ID($3, module, false, false);
			...
			if ( val_var )
				...
				val_var = zeek::install_ID($5, module, false, false);
	...
	|
		TOK_FOR '(' '[' local_id_list ']' ',' TOK_ID TOK_IN expr ')'
			...
			auto val_var = zeek::lookup_ID($7, module);
			...
			if ( val_var )
				...
				val_var = zeek::install_ID($7, module, false, false);
...
local_id:
		TOK_ID
			...
			if ( $$ )
				...
				{
				$$ = zeek::install_ID($1, zeek::current_module.c_str(),
...
global_or_event_id:
		TOK_ID
			{
			set_location(@1);
			auto id = zeek::lookup_ID($1, zeek::current_module.c_str(), false,
			                    defining_global_ID);
			$$ = id.release();
			...
			if ( $$ )
				...
				const char* module_name =
					resolving_global_ID ?
						zeek::current_module.c_str() : 0;

				$$ = zeek::install_ID($1, module_name,
