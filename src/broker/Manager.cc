...
static inline zeek::Val* get_option(const char* option)
	{
	const auto& id = zeek::global_scope()->Find(option);
...
struct scoped_reporter_location {
	scoped_reporter_location(zeek::Frame* frame)
		{
...
bool Manager::PublishIdentifier(std::string topic, std::string id)
	...
	const auto& i = zeek::global_scope()->Find(id);
...
zeek::RecordVal* Manager::MakeEvent(val_list* args, zeek::Frame* frame)
	{
...
bool Manager::ProcessIdentifierUpdate(broker::zeek::IdentifierUpdate iu)
	...
	const auto& id = zeek::global_scope()->Find(id_name);
