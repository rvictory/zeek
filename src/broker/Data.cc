...
struct val_converter {
	...
	result_type operator()(broker::vector& a)
		...
		if ( type->Tag() == zeek::TYPE_VECTOR )
			...
			const auto& id = zeek::global_scope()->Find(*name);
			if ( ! id )
				return nullptr;
...
zeek::EnumValPtr bro_broker::get_data_type(zeek::RecordVal* v, zeek::Frame* frame)
	{
	return caf::visit(data_type_getter{}, opaque_field_to_data(v, frame));
	}

broker::data& bro_broker::opaque_field_to_data(zeek::RecordVal* v, zeek::Frame* f)
	{
