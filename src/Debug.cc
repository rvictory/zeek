...
DebuggerState::DebuggerState()
	...
	dbg_locals = new zeek::Frame(1024, /* func = */ nullptr, /* fn_args = */ nullptr);
	}
...
static void parse_function_name(vector<ParseLocationRec>& result,
	...
	{ // function name
	const auto& id = zeek::lookup_ID(s.c_str(), zeek::current_module.c_str());

	if ( ! id )
		{
		string fullname = make_full_var_name(zeek::current_module.c_str(), s.c_str());
...
string get_context_description(const zeek::detail::Stmt* stmt, const zeek::Frame* frame)
	{
	ODesc d;
...
int dbg_handle_debug_input()
	...
	zeek::Frame* curr_frame = g_frame_stack.back();
	const BroFunc* func = curr_frame->GetFunction();
	if ( func )
		zeek::current_module = extract_module_name(func->Name());
	else
		zeek::current_module = GLOBAL_MODULE_NAME;
...
bool pre_execute_stmt(zeek::detail::Stmt* stmt, zeek::Frame* f)
	{
	if ( ! g_policy_debug ||
...
bool post_execute_stmt(zeek::detail::Stmt* stmt, zeek::Frame* f, zeek::Val* result, stmt_flow_type* flow)
	{
...
zeek::ValPtr dbg_eval_expr(const char* expr)
	...
	zeek::Frame* frame = g_frame_stack[frame_idx];
	if ( ! (frame)  )
	...
	if ( func )
		{
		Ref(func->GetScope());
		zeek::push_existing_scope(func->GetScope());
		}
